...
const Columns = ({col, id, setColumnDraggable}) => {
   const dispatch = useDispatch()
   const taskSlice = useSelector((state)=> state.taskSlice) 
   const taskDraggable = useSelector((state)=> state.taskSlice.taskDraggable) 
    ...
    {/* {taskSlice.showTaskDetailsPopup && <ShowDetailsPopup/>}  */}
        <div id={id} className="border border-white rounded-lg min-h-[60vh] w-[80%] p-3">
          ...
          {col.map((task)=>(
            ...
            key={task._id}
            >
              <div className="font-bold bg-white text-center max-w-[100%] rounded-lg mb-2 h-20" onMouseEnter={()=>setColumnDraggable(true) } onMouseLeave={()=>setColumnDraggable(false)}>
